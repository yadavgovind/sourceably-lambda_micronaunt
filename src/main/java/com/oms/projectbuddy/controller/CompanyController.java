//package com.oms.projectbuddy.controller;
//
//import com.oms.projectbuddy.model.request.*;
//
//
//import com.oms.projectbuddy.config.TokenProvider;
//import com.oms.projectbuddy.exception.SourceablyCustomeException;
//import com.oms.projectbuddy.model.CompanyRegistration;
//import com.oms.projectbuddy.model.response.CustomResponseMessage;
//import com.oms.projectbuddy.model.response.EntityResponse;
//import com.oms.projectbuddy.model.response.LoginResponse;
//import com.oms.projectbuddy.services.ICompanyRegistrationService;
//import com.oms.projectbuddy.utils.ExceptionUtils;
//import io.micronaut.http.annotation.Controller;
//import io.micronaut.scheduling.TaskExecutors;
//import io.micronaut.scheduling.annotation.ExecuteOn;
//import jakarta.inject.Inject;
//
//
//@ExecuteOn(TaskExecutors.IO)
//@Controller("/api")
////@Api(value = "Registration", description = "company Registration", tags = {"Company Signup"})
//public class CompanyController {
//    @Inject
//    private   ICompanyRegistrationService iCompanyRegistrationService;
//    @Inject
//    private TokenProvider jwtTokenUtil;
//    @Inject
//    private AuthenticationManager authenticationManager;
//
//    @Post("/registration")
//    public Object registration(@Body CompanyRegistrationRequest registrationRequest) {
//        try {
//            return
//                    new EntityResponse(iCompanyRegistrationService.signUp(registrationRequest), 0);
//        } catch (Exception e) {
//            return new CustomResponseMessage(e.getMessage(), -1), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//    @Get("/getCompanyById")
//    public Object getCompanyById( Long id) throws Exception {
//    	return new EntityResponse(iCompanyRegistrationService.getById(id), 0);
//    }
//
//    @Get("/getAllCompany")
//    public Object getAllCompany() throws Exception{
//    	return new EntityResponse(iCompanyRegistrationService.getAllCompany(), 0);
//    }
//
//    @Post("/companyLogin")
//    public Object login(@Body LoginRequest loginRequest) throws Exception {
//        try {
//            final Authentication authentication = authenticate(loginRequest.getEmail().toLowerCase(), loginRequest.getPassword());
//
//            final UserDetails userDetails = iCompanyRegistrationService.loadUserByUsername(loginRequest.getEmail().toLowerCase());
//            CompanyRegistration userFromAuth = iCompanyRegistrationService.findByEmail(userDetails.getUsername().toLowerCase());
//            if (!userFromAuth.getIsActive() || userFromAuth.getIsDeleted()) {
//            	ExceptionUtils.throwException("User Not Active");
//            }
//            if (userFromAuth.getIsFirstLogin()==null || !userFromAuth.getIsFirstLogin()) {
//                iCompanyRegistrationService.modifyLogin(userFromAuth.getCompanyId());
//            }
//            final String token = jwtTokenUtil.generateToken(authentication,userFromAuth.getRegistrationType(),userFromAuth.getUserId());
//            LoginResponse loginResponse = new LoginResponse();
//            loginResponse.setToken(token);
//            loginResponse.setId(userFromAuth.getCompanyId());
//            loginResponse.setIsEmailVerified(userFromAuth.getIsEmailVerified());
//            loginResponse.setIsMobileVerified(userFromAuth.getIsMobileVerified());
//            loginResponse.setName(userFromAuth.getFirstName());
//            loginResponse.setRegistrationType(userFromAuth.getRegistrationType());
//            loginResponse.setUserId(userFromAuth.getUserId());
//            loginResponse.setIsFirstLogin(userFromAuth.getIsFirstLogin());
//            return new EntityResponse(loginResponse, 0);
//        } catch (Exception e) {
//            return new EntityResponse("Please Check Username and Password", -1),
//                    HttpStatus.UNAUTHORIZED);
//
//        }
//    }
//
//    @Post("/companyLogout")
//    public Object logout( String token) {
//        try {
//            iCompanyRegistrationService.logout(token);
//            return new CustomResponseMessage("Logout", 0);
//
//        } catch (Exception e) {
//        	throw new SourceablyCustomeException("Not Saved", HttpStatus.BAD_REQUEST);
//        }
//    }
//
//    @Put("/changePassword")
//    public Object changePassword(@Body ChangePassword changePassword) {
//        try {
//            return new EntityResponse(iCompanyRegistrationService.changePassword(changePassword), 0);
//        } catch (Exception e) {
//        	throw new SourceablyCustomeException(e.getMessage(), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//
//    private Authentication authenticate(String username, String password) throws Exception {
//        try {
//            return authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(username, password));
//        } catch (Exception e) {
//        	throw new SourceablyCustomeException("INVALID_CREDENTIALS", HttpStatus.UNPROCESSABLE_ENTITY);
//        }
//    }
//
//    @Post("/forgotPassword")
//    public Object forgotPassword( String email,  String link) {
//        try {
//            boolean exist = iCompanyRegistrationService.isEmailId(email);
//            if (exist) {
//               // String companyId = iCompanyRegistrationService.forgotPassword(email, link);
//                return new EntityResponse("Reset link send on Registered Email", 0);
//            } else {
//            	throw new SourceablyCustomeException("Email does not exist", HttpStatus.UNPROCESSABLE_ENTITY);
//            }
//        } catch (Exception e) {
//            throw new SourceablyCustomeException(e.getMessage(), HttpStatus.BAD_REQUEST);
//        }
//
//    }
//
//    @Put("/resetPassword")
//    public Object resetPassword(@Body ResetPassword resetPassword) {
//        try {
//            boolean exist = iCompanyRegistrationService.isEmailId(resetPassword.getEmail());
//            if (exist) {
//                return new EntityResponse(iCompanyRegistrationService.resetPassword(resetPassword), 0);
//            } else {
//                return new CustomResponseMessage("Email does not exist", -1);
//            }
//        } catch (Exception e) {
//            throw new SourceablyCustomeException(e.getMessage(), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//    @Get("/verifyOtp")
//    public Object verifyOtp( int mobileOtp,
//                                        String mobileNumber,  String email) {
//        try {
//            CompanyRegistration companyRegistration = getUserFromToken(email);
//            companyRegistration.setMobileNumber(mobileNumber);
//            return new EntityResponse(iCompanyRegistrationService.verifyOtp(mobileOtp, companyRegistration), 0);
//        } catch (Exception e) {
//            throw new SourceablyCustomeException(e.getMessage(), HttpStatus.UNPROCESSABLE_ENTITY);
//        }
//    }
//
//    @Post("/resendEmailOtp")
//    public Object resendEmailOtp( String email) {
//        try {
//            if (iCompanyRegistrationService.findByEmail(email) != null) {
//                iCompanyRegistrationService.resendEmailOtp(email);
//                return new EntityResponse("OTP Sent on Registered Email", 0);
//            } else
//            	throw new SourceablyCustomeException("Email does not exist";
//        } catch (Exception e) {
//        	throw new SourceablyCustomeException(e.getMessage(), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//    @Post("/resendMobileOtp")
//    public Object resendMobileOtp( String mobileNumber) {
//        try {
//            boolean exist = iCompanyRegistrationService.isMobileNumber(mobileNumber);
//            if (exist) {
//                iCompanyRegistrationService.resendMobOtp(mobileNumber);
//                return new EntityResponse("OTP Sent on Registered Mobile", 0);
//            } else
//            throw new SourceablyCustomeException("Mobile number does not exist";
//        } catch (Exception e) {
//            throw new SourceablyCustomeException(e.getMessage(), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//    @Post("/resendMobileOtpForUs")
//    public Object resendMobileOtpForUs( String mobileNumber) {
//        try {
//            iCompanyRegistrationService.resendMobOtpForUs(mobileNumber);
//            return new EntityResponse("OTP Sent on Registered Mobile", 0);
//        } catch (Exception e) {
//        	throw new SourceablyCustomeException(e.getMessage(), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//
//    private CompanyRegistration getUserFromToken(String email) throws Exception {
//        try {
//            UserDetails userDetails = (UserDetails) SecurityContextHolder.getContext().getAuthentication()
//                    .getPrincipal();
//            String username = userDetails.getUsername();
//            return iCompanyRegistrationService.findByEmail(username);
//        } catch (Exception e) {
//            return iCompanyRegistrationService.findByEmail(email);
//        }
//
//    }
//
//    @Post("/sendMobEmailOtp")
//    public Object sendEmailMobOtp( String mobileNumber,  String email) {
//        try {
//            return new EntityResponse(iCompanyRegistrationService.sendMobOtp(email, mobileNumber), 0);
//
//        } catch (Exception e) {
//            return new CustomResponseMessage(e.getMessage(), -1), HttpStatus.BAD_REQUEST);
//        }
//    }
//
//    @Post("/saveLogo")
//    public Object saveLogo(@Body CompanyLogoRequest fileRequest) {
//        try {
//            return new CustomResponseMessage(iCompanyRegistrationService.saveCompanyLogo(fileRequest), 0);
//        } catch (Exception e) {
//            return new CustomResponseMessage(e.getMessage(), -1);
//        }
//    }
//
//    @Put("/updateLogo")
//    public Object updateLogo(@Body CompanyLogoRequest fileRequest) {
//        try {
//            return new CustomResponseMessage(iCompanyRegistrationService.updateCompanyLogo(fileRequest), 0);
//        } catch (Exception e) {
//            return new CustomResponseMessage(e.getMessage(), -1);
//        }
//    }
//
//    @Get("/getLogoById")
//    public Object getLogoById( String userId) {
//        try {
//            return new EntityResponse(iCompanyRegistrationService.getCompanyLogoById(userId), 0);
//        } catch (Exception e) {
//            return new CustomResponseMessage(e.getMessage(), -1);
//        }
//    }
//
//    @Delete("/deleteLogo")
//    public Object deleteLogo( String userId) {
//        try {
//            return new EntityResponse(iCompanyRegistrationService.deleteCompanyLogo(userId), 0);
//        } catch (Exception e) {
//            return new CustomResponseMessage(e.getMessage(), -1);
//        }
//    }
//}
